...
parser.add_argument('-fa', '--fasta', required=True, help='FastA file name goes here')
parser.add_argument('-P', '--pseudo', type=int, default=DEFAULT_PSEUDO, required=False, help='pseudo count')
parser.add_argument('-t', '--threshold', type=float, default=DEFAULT_THRESHOLD, required=False, help='%MAX LOD score threshold')
parser.add_argument('-s', '--scramflag', action='store_true', default=False, required=False, help='use scrambled input sequence')
parser.add_argument('-p', '--pwmdir', type=str, default=DEFAULT_PWMDIR, required=False, help='PWMs to use')
parser.add_argument('-N', '--number', type=int, default=DEFAULT_NUMBER, required=False, help='shuffle run number')
parser.add_argument('-C', '--coord', action='store_true', default=False, required=False, help='scan for coordinates only')
parser.add_argument('-d', '--distance', type=int, default=DEFAULT_DISTANCE, required=False, help='max PWM distance')
...
bg=MOODS.flatbg()
matrix_max_list=[]
matrix_name_list = [filename for filename in os.listdir(pwm_directory) if filename[-5:] == '.jpwm']
matrix_list= [ MOODS.load_matrix(pwm_directory+filename) for filename in matrix_name_list]
matrix_list=[MOODS.count_log_odds(matrix,bg,pseudo,log_base=2) for matrix in matrix_list]
matrix_max_list=[MOODS.max_score(matrix) for matrix in matrix_list]
threshold_list=[(float(threshold)*matrix_max) for matrix_max in matrix_max_list]
...
for seq_id in seq_list:
	...
	result_list = MOODS.search(seq,matrix_list,threshold_list,convert_log_odds=False,threshold_from_p=False,both_strands=True,log_base=2)
	for (matrix,matrix_name,result,matrix_max) in zip(matrix_list,matrix_name_list,result_list,matrix_max_list):
		l=len(matrix[0])
		for (pos,score) in result:
			...
			if pos < 0:
				...
				strand="-"
				hit_seq="".join(reversed(map(lambda s: {'A':'T','T':'A','C':'G','G':'C','N':'N'}[s], hit_seq.upper())))
			else:
			...
			if hit_seq=="":
				hit_seq="N"*l
			hit_list.append([seq_id,seq_chr,str(start_coord),str(end_coord),str(matrix_name),str(percent_max),str(strand),hit_seq,str(score),str(matrix_max)])
	hit_list.sort(key=operator.itemgetter(0,1,2,3))
	for i in range(0,len(hit_list)-1):
		SEQID_1,CHR_1,START_1,END_1,PWMID_1,SCORE_1,STRAND_1,MATCHSEQ_1,LODSCORE_1,MAXPWMSCORE_1=hit_list[i]
		for j in range(i+1,len(hit_list)):
			SEQID_2,CHR_2,START_2,END_2,PWMID_2,SCORE_2,STRAND_2,MATCHSEQ_2,LODSCORE_2,MAXPWMSCORE_2=hit_list[j]
			...
			if distance > max_distance:
				...
				if scram_flag==False and coord_flag==True:
					...
					if distance >= min_distance:
						cmseq_dict[key_string].append([str(CHR_1),str(cm_start),str(cm_end),str(p1)+"|"+str(p2)+"|"+str(stereo_pair)+"|"+str(distance),str(SCORE_1)+"|"+str(SCORE_2),str(cm_strand)])
	if scram_flag==False and coord_flag==True:
